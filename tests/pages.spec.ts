...
test('receives data from the controllers as props', async ({ page }) => {
  await page.goto('/')
  const lunaProps = await page.evaluate(() => (window as any)._luna_props)

  await expect(lunaProps.example).toEqual('FooBar')
})
...
test.describe('persistent layouts', () => {
  ...
  layoutData.forEach(({ method }) => {
    test(`can have a persistent layout (${method})`, async ({ page }) => {
      ...
      const layoutAId = await page.evaluate(() => (window as any)._luna_layout_id)
      ...
      const layoutBId = await page.evaluate(() => (window as any)._luna_layout_id)
  ...
  layoutData.forEach(({ method }) => {
    test(`can create more complex layout arrangements using nested layouts (${method})`, async ({ page }) => {
      ...
      const layoutAId = await page.evaluate(() => (window as any)._luna_layout_id)
      const nestedLayoutAId = await page.evaluate(() => (window as any)._luna_nested_layout_id)
      await expect(layoutAId).not.toBeNull()
      ...
      const layoutBId = await page.evaluate(() => (window as any)._luna_layout_id)
      const nestedLayoutBId = await page.evaluate(() => (window as any)._luna_nested_layout_id)
      await expect(layoutBId).toEqual(layoutAId)
  ...
  test('has the page props available within the persistent layout', async ({ page }) => {
    ...
    const pageProps = await page.evaluate(() => (window as any)._luna_page_props)
    const layoutProps = await page.evaluate(() => (window as any)._luna_site_layout_props)
  ...
  test('has the page props available within all nested persistent layouts', async ({ page }) => {
    ...
    const pageProps = await page.evaluate(() => (window as any)._luna_page_props)
    const layoutProps = await page.evaluate(() => (window as any)._luna_site_layout_props)
    const nestedLayoutProps = await page.evaluate(() => (window as any)._luna_nested_layout_props)

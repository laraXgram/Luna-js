...
function infiniteScrollRequests() {
  return requests.requests.filter((req) => {
    return (
      ...
      !!req.headers()['x-luna'] &&
      !!req.headers()['x-luna-partial-data']
    )
  })
}
...
test.describe('URL query string management', () => {
  ...
  test('it preserves relative URL format when updating query string', async ({ page }) => {
    ...
    // Verify the internal Luna page URL is still relative
  ...
  test('it preserves absolute URL format when updating query string', async ({ page }) => {
    ...
    // Verify the internal Luna page URL is still absolute
    const updatedUrl = await page.evaluate(() => window.testing.pageUrl)
    expect(updatedUrl).toContain('page=2')
...
test('it preserves validation errors when InfiniteScroll loads more data', async ({ page }) => {
  ...
  const loadNextResponsePromise = page.waitForResponse(
    (response) => response.request().headers()['x-luna-partial-data'] === 'users' && response.status() === 200,
  )

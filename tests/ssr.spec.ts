...
test.describe('Head title escaping', () => {
  ...
  test('it runs the titleCallback with the page during SSR', async ({ page }) => {
    ...
    expect(html).toContain('<title data-luna="">SSR Head Title | From Props</title>')
  })
})
...
test.describe('server head', () => {
  test('it renders server-provided head elements in the SSR output', async ({ page }) => {
    ...
    expect(html).toContain('<title data-luna="title">Server Head SSR</title>')
    expect(html).toContain('<meta data-luna="description" name="description" content="Rendered on the server">')
    expect(html).toContain('<link data-luna="canonical" rel="canonical" href="https://example.com/ssr">')

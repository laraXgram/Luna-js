...
test.describe('Deferred Props Cancellation', () => {
  test('it cancels in-flight deferred props when navigating to a different URL', async ({ page }) => {
    ...
    page.on('requestfailed', (req) => {
      if (req.url().includes('/a') && req.headers()['x-luna-partial-data']) {
        cancelledFromA.push(req.headers()['x-luna-partial-data'])
      }
    })

    page.on('response', (res) => {
      const prop = res.request().headers()['x-luna-partial-data']
  ...
  test('it does not cancel deferred props for same-URL operations', async ({ page }) => {
    ...
    page.on('requestfailed', (req) => {
      if (req.url().includes('rapid-navigation') && req.headers()['x-luna-partial-data']) {
        cancelled.push(req.headers()['x-luna-partial-data'])
      }
    })
  ...
  test('it preserves prefetch requests when navigating', async ({ page }) => {
    ...
    page.on('requestfailed', (req) => {
      ...
      } else if (req.headers()['x-luna-partial-data']) {
        cancelledDeferred.push(req.headers()['x-luna-partial-data'])
      }
    })
  ...
  test('it cancels deferred props on back navigation', async ({ page }) => {
    ...
    page.on('requestfailed', (req) => {
      if (req.url().includes('/b') && req.headers()['x-luna-partial-data']) {
        cancelledFromB.push(req.headers()['x-luna-partial-data'])
      }
    })

...
test('it pairs client-side push visitId with navigate', async ({ page }) => {
  ...
  await listenForGlobalMessages(page, 'luna:clientVisit')

  await page.getByRole('button', { name: 'Push', exact: true }).click()

  const clientVisitMessages = await waitForGlobalMessages(page, 'luna:clientVisit', 1)
...
test('it records client-side replace visitIds without navigate events', async ({ page }) => {
  ...
  const replaceClientVisitMessages = await waitForGlobalMessages(page, 'luna:clientVisit', 1)
  const replaceNavigateMessages = await waitForGlobalMessages(page, 'luna:navigate')
  ...
  const clientVisitMessages = await waitForGlobalMessages(page, 'luna:clientVisit', 2)
  const navigateMessages = await waitForGlobalMessages(page, 'luna:navigate')

...
test.describe('layout props', () => {
  ...
  test('it preserves named layout instances across navigations', async ({ page }) => {
    ...
    const appLayoutId = await page.evaluate(() => window._luna_app_layout_id)
    const contentLayoutId = await page.evaluate(() => window._luna_content_layout_id)
    ...
    const appLayoutIdAfter = await page.evaluate(() => window._luna_app_layout_id)
    const contentLayoutIdAfter = await page.evaluate(() => window._luna_content_layout_id)

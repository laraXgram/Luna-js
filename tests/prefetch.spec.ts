...
test('displays the error modal correctly when prefetching a non-Luna response', async ({ page }) => {
  await page.goto('prefetch/after-error')

  const prefetchPromise = page.waitForResponse('/non-luna')
  await page.getByRole('button', { name: 'Prefetch Non-Luna' }).click()
  await prefetchPromise

  await page.getByRole('button', { name: 'Visit Non-Luna' }).click()

...
const assertGlobalErrorEvent = async (event) => {
  await assertIsGlobalEvent(event, 'luna:error', false)
  await assertErrorsObject(event.detail.errors)
}

const assertGlobalSuccessEvent = async (event) => {
  await assertIsGlobalEvent(event, 'luna:success', false)
...
const assertGlobalFinishEvent = async (event) => {
  await assertIsGlobalEvent(event, 'luna:finish', false)
...
test.describe('Events', () => {
  ...
  test.describe('visitId', () => {
    test('it uses one id across the server visit lifecycle', async ({ page }) => {
      await listenForGlobalMessages(page, 'luna:before')
      await listenForGlobalMessages(page, 'luna:start')
      await listenForGlobalMessages(page, 'luna:success')
      await listenForGlobalMessages(page, 'luna:navigate')
      await listenForGlobalMessages(page, 'luna:finish')

      await page.getByRole('link', { exact: true, name: 'Navigate Event' }).click()

      const beforeMessages = await waitForGlobalMessages(page, 'luna:before', 1)
      const startMessages = await waitForGlobalMessages(page, 'luna:start', 1)
      const successMessages = await waitForGlobalMessages(page, 'luna:success', 1)
      const navigateMessages = await waitForGlobalMessages(page, 'luna:navigate', 1)
      const finishMessages = await waitForGlobalMessages(page, 'luna:finish', 1)
    ...
    test('it assigns distinct ids to distinct server visits', async ({ page }) => {
      ...
      await waitForGlobalMessages(page, 'luna:before', 1)
      await page.goBack()
      await page.locator('.navigate').dispatchEvent('click')

      const beforeMessages = await waitForGlobalMessages(page, 'luna:before', 2)
    ...
    test('it leaves navigate events without a visit id for popstate navigations', async ({ page }) => {
      ...
      await page.getByRole('link', { exact: true, name: 'Navigate Event' }).click()
      await waitForGlobalMessages(page, 'luna:navigate', 1)
      await page.goBack()

      const navigateMessages = await waitForGlobalMessages(page, 'luna:navigate', 2)
  ...
  test.describe('Listeners', () => {
    ...
    test.describe('Luna.once', () => {
      ...
      test('returns a callback that can be used to remove the global listener before it fires', async ({ page }) => {
        await page.getByRole('link', { name: 'Remove Luna Once Listener' }).click()

        const messages = await waitForMessages(page, 2)

        await expect(messages[0]).toBe('Removing Luna.once Listener')
        await expect(messages[1]).toBe('onBefore')
      })
    })
  })
  ...
  test.describe('Hooks', () => {
    test.describe('before', () => {
      test('fires the event when a request is about to be made', async ({ page }) => {
        ...
        const globalMessages = await page.evaluate(() => (window as any).globalMessages['luna:before'])

        // Local Event Callback
        await expect(messages[0]).toBe('onBefore')
        await assertVisitObject(messages[1])

        // Global Luna Event Listener
        await expect(messages[2]).toBe('Luna.on(before)')

        await expect(globalMessages).toHaveLength(1)

        await assertIsGlobalEvent(globalMessages[0], 'luna:before', true)
      ...
      test.describe('Global Luna.on', () => {
        test('can prevent the visit by returning false ', async ({ page }) => {
          await page
            .getByRole('link', { exact: true, name: 'Before Event - Prevent globally using Luna Event Listener' })
            .click()

          const messages = await waitForMessages(page, 3)
          await expect(messages[0]).toBe('onBefore')
          await expect(messages[1]).toBe('addEventListener(luna:before)')
          await expect(messages[2]).toBe('Luna.on(before)')
        })
      })
      ...
      test.describe('Global addEventListener', () => {
        test('can prevent the visit by using preventDefault', async ({ page }) => {
          ...
          const messages = await waitForMessages(page, 3)
          await expect(messages[0]).toBe('onBefore')
          await expect(messages[1]).toBe('Luna.on(before)')
          await expect(messages[2]).toBe('addEventListener(luna:before)')
        })
      })
    })
    ...
    test.describe('start', () => {
      test('fires when the request has started', async ({ page }) => {
        ...
        const globalMessages = await waitForGlobalMessages(page, 'luna:start', 1)

        await assertIsGlobalEvent(globalMessages[0], 'luna:start', false)
        await assertVisitObject(globalMessages[0].detail.visit)

        // Global Luna Event Listener
        await expect(messages[0]).toBe('Luna.on(start)')

        // Global Native Event Listener
        await expect(messages[2]).toBe('addEventListener(luna:start)')
    ...
    test.describe('progress', () => {
      test('fires when the request has files and upload progression occurs', async ({ page }) => {
        await listenForGlobalMessages(page, 'luna:progress')
        await clickAndWaitForResponse(page, 'Progress Event')

        const messages = await waitForMessages(page, 6)
        const globalMessages = await waitForGlobalMessages(page, 'luna:progress', 1)

        await assertIsGlobalEvent(globalMessages[0], 'luna:progress', false)
        await assertProgressObject(globalMessages[0].detail.progress)

        // Global Luna Event Listener
        await expect(messages[0]).toBe('Luna.on(progress)')

        // Global Native Event Listener
        await expect(messages[2]).toBe('addEventListener(luna:progress)')
    ...
    test.describe('error', () => {
      test('fires when the request finishes with validation errors', async ({ page }) => {
        await listenForGlobalMessages(page, 'luna:error')
        await clickAndWaitForResponse(page, 'Error Event', 'events/errors')

        const messages = await waitForMessages(page, 6)
        const globalMessages = await waitForGlobalMessages(page, 'luna:error', 1)

        await assertGlobalErrorEvent(globalMessages[0])

        // Global Luna Event Listener
        await expect(messages[0]).toBe('Luna.on(error)')

        // Global Native Event Listener
        await expect(messages[2]).toBe('addEventListener(luna:error)')
  ...
  test.describe('success', () => {
    test('fires when the request finished without validation errors', async ({ page }) => {
      ...
      const messages = await waitForMessages(page, 6)
      const globalMessages = await waitForGlobalMessages(page, 'luna:success', 1)

      await assertGlobalSuccessEvent(globalMessages[0])

      // Global Luna Event Listener
      await expect(messages[0]).toBe('Luna.on(success)')

      // Global Native Event Listener
      await expect(messages[2]).toBe('addEventListener(luna:success)')
  ...
  test.describe('httpException', () => {
    test('gets fired when a non-Luna response is received', async ({ page }) => {
      await listenForGlobalMessages(page, 'luna:httpException')
      await clickAndWaitForResponse(page, 'HTTP Exception Event', 'non-luna')

      const messages = await waitForMessages(page, 5)
      const globalMessages = await waitForGlobalMessages(page, 'luna:httpException', 1)

      await assertIsGlobalEvent(globalMessages[0], 'luna:httpException', true)
      ...
      // Global Luna Event Listener
      await expect(messages[1]).toBe('Luna.on(httpException)')

      // Global Native Event Listener
      await expect(messages[3]).toBe('addEventListener(luna:httpException)')
    })

    test('can prevent the default behavior by returning false from the visit callback', async ({ page }) => {
      await listenForGlobalMessages(page, 'luna:httpException')
      await clickAndWaitForResponse(page, 'HTTP Exception Event (Prevent)', 'non-luna')

      const messages = await waitForMessages(page, 2)
      const globalMessages = await waitForGlobalMessages(page, 'luna:httpException')
  ...
  test.describe('httpException with Luna error page response', () => {
    test('it fires when the server returns a valid Luna response with an error status code', async ({ page }) => {
      await listenForGlobalMessages(page, 'luna:httpException')
      await clickAndWaitForResponse(page, 'HTTP Exception Event (Luna Response)', 'luna-error-page')
    ...
    test('it suppresses the error page navigation when the visit callback returns false', async ({ page }) => {
      await listenForGlobalMessages(page, 'luna:httpException')
      await clickAndWaitForResponse(page, 'HTTP Exception Event (Luna Response Prevent)', 'luna-error-page')
  ...
  test.describe('networkError', () => {
    test('gets fired when an unexpected situation occurs (e.g. network disconnect)', async ({ page }) => {
      await listenForGlobalMessages(page, 'luna:networkError', true)
      await page.getByRole('link', { exact: true, name: 'Network Error Event' }).click()

      const messages = await waitForMessages(page, 5)
      const globalMessages = await waitForGlobalMessages(page, 'luna:networkError', 1)

      await assertIsGlobalEvent(globalMessages[0], 'luna:networkError', true)
      ...
      // Global Luna Event Listener
      await expect(messages[1]).toBe('Luna.on(networkError)')

      // Global Native Event Listener
      await expect(messages[3]).toBe('addEventListener(luna:networkError)')
    })
  ...
  test.describe('finish', () => {
    test('fires when the request completes', async ({ page }) => {
      ...
      const messages = await waitForMessages(page, 6)
      const globalMessages = await waitForGlobalMessages(page, 'luna:finish', 1)

      await assertGlobalFinishEvent(globalMessages[0])

      // Global Luna Event Listener
      await expect(messages[0]).toBe('Luna.on(finish)')

      // Global Native Event Listener
      await expect(messages[2]).toBe('addEventListener(luna:finish)')
  ...
  test.describe('navigate', () => {
    test('fires when the page navigates away after a successful request', async ({ page }) => {
      ...
      const globalMessages = await waitForGlobalMessages(page, 'luna:navigate', 1)

      await assertIsGlobalEvent(globalMessages[0], 'luna:navigate', false)
      await assertPageObject(globalMessages[0].detail.page)

      // Global Luna Event Listener
      await expect(messages[0]).toBe('Luna.on(navigate)')

      // Global Native Event Listener
      await expect(messages[2]).toBe('addEventListener(luna:navigate)')
    })
  })
...
test.describe('Lifecycles', () => {
  ...
  test('fires all expected events in the correct order on a successful request', async ({ page }) => {
    ...
    await expect(messages[4]).toBe('Luna.on(start)')
    await expect(messages[5]).toBe('addEventListener(luna:start)')
    await expect(messages[6]).toBe('onStart')
    await expect(messages[7]).toBe('Luna.on(progress)')
    await expect(messages[8]).toBe('addEventListener(luna:progress)')
    await expect(messages[9]).toBe('onProgress')
    await expect(messages[10]).toBe('Luna.on(success)')
    await expect(messages[11]).toBe('addEventListener(luna:success)')
    await expect(messages[12]).toBe('onSuccess')
    await expect(messages[13]).toBe('Luna.on(finish)')
    await expect(messages[14]).toBe('addEventListener(luna:finish)')
  ...
  test('fires all expected events in the correct order on an error request', async ({ page }) => {
    ...
    await expect(messages[8]).toBe('addEventListener(luna:progress)')
    await expect(messages[9]).toBe('onProgress')
    await expect(messages[10]).toBe('Luna.on(navigate)')
    await expect(messages[11]).toBe('addEventListener(luna:navigate)')
    await expect(messages[12]).toBe('Luna.on(error)')
    await expect(messages[13]).toBe('addEventListener(luna:error)')
    await expect(messages[14]).toBe('onError')
    await expect(messages[15]).toBe('Luna.on(finish)')
    await expect(messages[16]).toBe('addEventListener(luna:finish)')
  ...
  test.describe('Cancelling', () => {
    test('cancels a visit before it completes', async ({ page }) => {
      ...
      await expect(messages[12]).toBe('Luna.on(finish)')
      await expect(messages[13]).toBe('addEventListener(luna:finish)')

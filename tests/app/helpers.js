...
const processPartialProps = (req, data) => {
  const partialDataHeader = req.headers['x-luna-partial-data'] || ''
  const partialExceptHeader = req.headers['x-luna-partial-except'] || ''
  const partialComponentHeader = req.headers['x-luna-partial-component'] || ''
...
module.exports = {
  ...
  render: (req, res, data) => {
    ...
    if (data.alwaysProps) {
      // To simulate Luna::always() in the LaraGram adapter...
  ...
  renderUnified: (req, res, data) => {
    ...
    const html = fs
      ...
      .toString()
      .replace('{{ headAttribute }}', data.component === 'Head/Dataset' ? 'data-luna' : 'luna')
  ...
  location: (res, href, version) => {
    res.status(409).header('X-Luna-Location', href)

    if (version) {
      res.header('X-Luna-Version', version)
    }

    return res.send('')
  },
  redirect: (res, href) => res.status(409).header('X-Luna-Redirect', href).send(''),
}

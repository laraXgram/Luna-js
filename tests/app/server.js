...
if (!adapters.includes(luna.package)) {
  throw new Error(`Invalid adapter package "${luna.package}". Expected one of: ${adapters.join(', ')}.`)
}

app.all('/non-luna', (req, res) =>
  res.status(200).send(`
    <!DOCTYPE html>
    <html>
      <head><title>Non-Luna Page</title></head>
      <body>
        <h1>This is a page that does not have the Luna app loaded.</h1>
        <p><a href="/navigate-non-luna">Go to Luna page</a></p>
      </body>
    </html>
  `),
)

app.post('/non-luna/xss', (req, res) =>
  res.status(200).send(`
    <!DOCTYPE html>
    <html>
...
app.get('/non-luna/download', (req, res) => {
...
app.get('/ssr/server-head', (req, res) =>
  luna.renderSSR(req, res, {
    ...
    props: {
      head: [
        '<title data-luna="title">Server Head SSR</title>',
        '<meta data-luna="description" name="description" content="Rendered on the server">',
        '<link data-luna="canonical" rel="canonical" href="https://example.com/ssr">',
      ],
    },
  }),
)
...
// createLunaApp (unified) test routes
app.get('/unified', (req, res) =>
  luna.renderUnified(req, res, {
    component: 'Home',
    props: {
      example: 'FooBar',
    },
...
app.get(/.*\.(?:js|css)$/, (req, res) => {
  const filePath = path.resolve(__dirname, '../../packages/', luna.package, 'test-app/dist', req.path.substring(1))
...
/**
 * Used for testing the Luna plugin is registered.
 * @see plugin.test.js
 */
...
app.all('/links/preserve-state-page-two', (req, res) =>
  luna.render(req, res, { component: 'Links/PreserveState', props: { foo: req.query.foo } }),
)
app.all('/links/preserve-scroll-page-two', (req, res) =>
  luna.render(req, res, { component: 'Links/PreserveScroll', props: { foo: req.query.foo } }),
)
app.all('/links/preserve-scroll-false-page-two', (req, res) =>
  luna.render(req, res, { component: 'Links/PreserveScrollFalse', props: { foo: req.query.foo } }),
)
...
app.all('/links/preserve-url-page-two', (req, res) =>
  luna.render(req, res, { component: 'Links/PreserveUrl', props: { foo: req.query.foo } }),
)
app.get('/links/as-warning/:method', (req, res) =>
  luna.render(req, res, { component: 'Links/AsWarning', props: { method: req.params.method } }),
)
app.get('/links/as-warning-false/:method', (req, res) =>
  luna.render(req, res, { component: 'Links/AsWarningFalse', props: { method: req.params.method } }),
)
app.get('/links/headers/version', (req, res) =>
  luna.render(req, res, { component: 'Links/Headers', version: 'example-version-header' }),
)
app.get('/links/sub', (req, res) => luna.render(req, res, { component: 'Links/PathTraversal' }))
app.get('/links/sub/sub', (req, res) => luna.render(req, res, { component: 'Links/PathTraversal' }))
app.get('/links/as-component/:page', (req, res) =>
  luna.render(req, res, { component: 'Links/AsComponent', props: { page: req.params.page } }),
)
app.get('/links/as-element/:page', (req, res) =>
  luna.render(req, res, { component: 'Links/AsElement', props: { page: req.params.page } }),
)
app.get('/links/cancel-sync-request/:page', (req, res) => {
  const page = req.params.page
  setTimeout(
    () => luna.render(req, res, { component: 'Links/CancelSyncRequest', props: { page } }),
    page == 3 ? 500 : 0,
  )
})
...
app.get('/instant-visit/deferred', (req, res) => {
  if (req.headers['x-luna-partial-data']?.includes('heavyData')) {
    return luna.render(req, res, {
...
app.get('/visits/proxy', (req, res) => {
  const timeout = req.headers['x-luna-partial-data'] ? 250 : 0
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        ...
        props: req.headers['x-luna-partial-data'] === 'sites' ? { sites } : { foo: new Date().toISOString() },
        deferredProps: req.headers['x-luna-partial-data'] ? {} : { default: ['sites'] },
      }),
    timeout,
  )
})
...
app.all('/visits/preserve-state-page-two', (req, res) =>
  luna.render(req, res, { component: 'Visits/PreserveState', props: { foo: req.query.foo } }),
)
app.all('/visits/preserve-scroll-page-two', (req, res) =>
  luna.render(req, res, { component: 'Visits/PreserveScroll', props: { foo: req.query.foo } }),
)
app.all('/visits/preserve-scroll-false-page-two', (req, res) =>
  luna.render(req, res, { component: 'Visits/PreserveScrollFalse', props: { foo: req.query.foo } }),
)
app.post('/visits/events-errors', (req, res) =>
  luna.render(req, res, { component: 'Visits/Events', props: { errors: { foo: 'bar' } } }),
)
app.get('/visits/headers/version', (req, res) =>
  luna.render(req, res, { component: 'Visits/Headers', version: 'example-version-header' }),
)
app.get('/visits/after-error/:page', (req, res) => luna.render(req, res, { component: 'Visits/AfterError' }))
...
app.get('/form-helper/errors/clear-on-resubmit', (req, res) =>
  luna.render(req, res, { component: 'FormHelper/ErrorsClearOnResubmit' }),
)
...
app.post('/precognition/error-sync', upload.any(), (req, res) => {
  ...
  setTimeout(() => {
    ...
    // Non-precognition: regular form submission with Luna error response
...
app.get('/visits/reload-on-mount', upload.any(), (req, res) => {
  if (req.headers['x-luna-partial-data']) {
    return luna.render(req, res, {
...
app.get('/persistent-layouts/shorthand/simple/page-a', (req, res) =>
  luna.render(req, res, { props: { foo: 'bar', baz: 'example' } }),
)
app.get('/persistent-layouts/shorthand/nested/page-a', (req, res) =>
  luna.render(req, res, { props: { foo: 'bar', baz: 'example' } }),
)

app.get('/default-layout', (req, res) => luna.render(req, res, { component: 'DefaultLayout/Index' }))
app.get('/layout-props/callback', (req, res) => luna.render(req, res, { props: { userName: 'Jane' } }))
app.get('/layout-props/callback-default', (req, res) => luna.render(req, res, { props: { userName: 'Jane' } }))
app.get('/layout-props/callback-static', (req, res) => luna.render(req, res))
app.get('/layout-props/static-object', (req, res) => luna.render(req, res))
app.get('/layout-props/callback-function-prop', (req, res) => luna.render(req, res))
app.get('/layout-props/callback-all-function-props', (req, res) => luna.render(req, res))
app.get('/layout-props/callback-component-prop', (req, res) => luna.render(req, res))

app.post('/events/errors', (req, res) =>
  luna.render(req, res, { component: 'Events', props: { errors: { foo: 'bar' } } }),
)
...
app.get('/poll/hook/manual', (req, res) => luna.render(req, res, { component: 'Poll/HookManual', props: {} }))
app.get('/poll/router/manual', (req, res) => luna.render(req, res, { component: 'Poll/RouterManual', props: {} }))
app.get('/poll/unchanged-data', (req, res) =>
  luna.render(req, res, { component: 'Poll/UnchangedData', props: { custom_prop: 'unchanged' } }),
)
...
app.get('/prefetch/after-error', (req, res) => luna.render(req, res, { component: 'Prefetch/AfterError' }))
app.get('/prefetch/test-page', (req, res) => luna.render(req, res, { component: 'Prefetch/TestPage' }))
app.get('/prefetch/wayfinder', (req, res) => luna.render(req, res, { component: 'Prefetch/Wayfinder' }))
app.get('/prefetch/navigate-event', (req, res) => luna.render(req, res, { component: 'Prefetch/NavigateEvent' }))
app.get('/prefetch/navigate-event/cached', (req, res) =>
  luna.render(req, res, { component: 'Prefetch/NavigateEventTarget', props: { label: 'cached' } }),
)
app.get('/prefetch/navigate-event/fresh', (req, res) =>
  luna.render(req, res, { component: 'Prefetch/NavigateEventTarget', props: { label: 'fresh' } }),
)
...
app.get('/when-visible', (req, res) => {
  ...
  if (req.headers['x-luna-partial-data'] || req.query.count) {
    setTimeout(page, 250)
  } else {
    page()
  }
...
app.get('/when-visible-merge-params', (req, res) => {
  const partialData = req.headers['x-luna-partial-data']
...
app.get('/merge-props', (req, res) => {
  luna.render(req, res, {
    ...
    ...(req.headers['x-luna-reset'] ? {} : { mergeProps: ['foo'] }),
  })
})
...
app.get('/deep-merge-props', (req, res) => {
  ...
  luna.render(req, res, {
    ...
    ...(req.headers['x-luna-reset'] ? {} : { deepMergeProps: ['foo', 'baz'] }),
  })
})

app.get('/complex-merge-selective', (req, res) => {
  const isReload = req.headers['x-luna-partial-component'] || req.headers['x-luna-partial-data']
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        component: 'DeferredProps/Page1',
        props: {
          foo: req.headers['x-luna-partial-data']?.includes('foo') ? { text: 'foo value' } : undefined,
          bar: req.headers['x-luna-partial-data']?.includes('bar') ? { text: 'bar value' } : undefined,
        },
      }),
    500,
  )
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        component: 'DeferredProps/WithPartialReload',
        props: {
          users: req.headers['x-luna-partial-data']?.includes('users') ? [{ id: 1, name: 'John Doe' }] : undefined,
        },
      }),
    500,
  )
...
app.get('/deferred-props/page-2', (req, res) => {
  ...
  if (req.headers['x-luna-partial-data']?.includes('baz')) {
    setTimeout(
      () =>
        luna.render(req, res, {
          component: 'DeferredProps/Page2',
  ...
  if (req.headers['x-luna-partial-data']?.includes('qux')) {
    setTimeout(
      () =>
        luna.render(req, res, {
          component: 'DeferredProps/Page2',
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        component: 'DeferredProps/Page3',
        props: {
          alpha: req.headers['x-luna-partial-data']?.includes('alpha') ? 'alpha value' : undefined,
          beta: req.headers['x-luna-partial-data']?.includes('beta') ? 'beta value' : undefined,
        },
      }),
    500,
  )
...
app.get('/deferred-props/many-groups', (req, res) => {
  ...
  const requestedProps = req.headers['x-luna-partial-data']
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        ...
        url: req.originalUrl,
        props: {
          results: req.headers['x-luna-partial-data']?.includes('results')
...
app.get('/svelte/props-and-page-store', (req, res) =>
  luna.render(req, res, { component: 'Svelte/PropsAndPageStore', props: { foo: req.query.foo || 'default' } }),
)
...
app.get('/remember/users', (req, res) => {
  ...
  luna.render(req, res, { component: 'FormHelper/RememberIndex', props: { users } })
})
...
app.get('/redirect-hash', (req, res) => {
  if (req.get('X-Luna')) {
    return luna.redirect(res, '/links/url-fragments#target')
  }
...
app.get('/location', ({ res }) => luna.location(res, '/dump/get'))
app.get('/visits/async-location-visit', (req, res) => {
  if (req.headers['x-simulate-version-change']) {
    return luna.location(res, req.originalUrl, 'updated-version')
  }

  if (req.headers['x-simulate-manual-location']) {
    return luna.location(res, '/dump/get')
  }

  luna.render(req, res, { component: 'Visits/AsyncLocationVisit' })
})
app.post('/redirect-external', (req, res) => luna.location(res, '/non-luna'))
app.post('/disconnect', (req, res) => res.socket.destroy())
app.post('/json', (req, res) => res.status(200).json({ foo: 'bar' }))
app.get('/luna-error-page', (req, res) => {
  const data = {
    component: 'ErrorPage',
    props: { status: 500 },
    url: req.originalUrl,
  ...
  if (req.get('X-Luna')) {
    ...
    res.header('X-Luna', true)
    return res.status(500).json(data)
  }

  return res.status(200).send(
    require('fs')
      .readFileSync(require('path').resolve(__dirname, '../../packages/', luna.package, 'test-app/dist/index.html'))
      .toString()
      .replace('{{ headAttribute }}', 'data-luna')
...
app.post('/form-component/events/delay', upload.any(), async (req, res) =>
  setTimeout(() => luna.render(req, res, { component: 'FormComponent/Events' }), 500),
)
...
app.get('/form-component/async-on-success', upload.any(), async (req, res) =>
  luna.render(req, res, { component: 'FormComponent/AsyncOnSuccess' }),
)
...
app.post('/form-component/events/success', async (req, res) =>
  luna.render(req, res, { component: 'FormComponent/Events' }),
)
...
app.post('/form-component/progress', async (req, res) =>
  setTimeout(() => luna.render(req, res, { component: 'FormComponent/Progress' }), 500),
)

app.get('/form-component/reset-on-error', (req, res) =>
  luna.render(req, res, { component: 'FormComponent/ResetAttributes/ResetOnError' }),
)
...
app.get('/form-component/reset-on-success', (req, res) =>
  luna.render(req, res, { component: 'FormComponent/ResetAttributes/ResetOnSuccess' }),
)
...
app.get('/form-component/reset-on-error-fields', (req, res) =>
  luna.render(req, res, { component: 'FormComponent/ResetAttributes/ResetOnErrorFields' }),
)
...
app.get('/form-component/reset-on-success-fields', (req, res) =>
  luna.render(req, res, { component: 'FormComponent/ResetAttributes/ResetOnSuccessFields' }),
)
...
app.get('/form-component/url/with/segements', (req, res) =>
  luna.render(req, res, { component: 'FormComponent/EmptyAction' }),
)
...
app.post('/form-component/wayfinder', (req, res) => {
  luna.render(req, res, { component: 'FormComponent/Wayfinder' })
})
...
app.post('/form-component/view-transition', (req, res) =>
  luna.render(req, res, { component: 'ViewTransition/PageB' }),
)
...
function renderInfiniteScroll(req, res, component, total = 40, orderByDesc = false, perPage = 15) {
  ...
  const partialReload = !!req.headers['x-luna-partial-data']
  const shouldAppend = req.headers['x-luna-infinite-scroll-merge-intent'] !== 'prepend'
...
app.get('/infinite-scroll', (req, res) => luna.render(req, res, { component: 'InfiniteScroll/Links' }))
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        ...
        ...(req.headers['x-luna-partial-data'] === 'users'
          ? { [shouldAppend ? 'mergeProps' : 'prependProps']: ['users.data'] }
          : {}),
        ...(req.headers['x-luna-partial-data'] === 'time' ? {} : { scrollProps: { users: scrollProp } }),
      }),
    partialReload ? 250 : 0,
  )
})
app.get('/infinite-scroll/dual-containers', (req, res) => {
  ...
  if (!partialReload || req.headers['x-luna-partial-data']?.includes('users1')) {
    props.users1 = users1Paginated
    scrollProps.users1 = users1ScrollProp
  }

  if (!partialReload || req.headers['x-luna-partial-data']?.includes('users2')) {
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        ...
        ...(req.headers['x-luna-reset'] ? {} : { [shouldAppend ? 'mergeProps' : 'prependProps']: ['users.data'] }),
        scrollProps: { users: scrollProp },
      }),
    partialReload ? 250 : 0,
  )
...
// Deferred scroll props test - simulates Luna::scroll()->defer()
...
// Optional scroll prop via WhenVisible - simulates Luna::optional(fn() => Luna::scroll(...))
...
app.post('/flash/events/without-data', (req, res) => luna.render(req, res, { component: 'Flash/Events' }))
...
app.get('/flash/location-visit', (req, res) => luna.location(res, '/flash/initial'))
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        component: 'Flash/WithDeferred',
        props: {
          data: req.headers['x-luna-partial-data']?.includes('data') ? 'Deferred data loaded' : undefined,
        },
      }),
    250,
  )
...
const getOncePropsData = (req, prop = 'foo') => {
  const isLunaRequest = !!req.headers['x-luna']
  const partialData = req.headers['x-luna-partial-data']?.split(',') ?? []
  const loadedOnceProps = req.headers['x-luna-except-once-props']?.split(',') ?? []
  const isPartialRequest = partialData.includes(prop)
  const hasPropAlready = loadedOnceProps.includes(prop)
  const shouldResolveProp = !isLunaRequest || isPartialRequest || !hasPropAlready
...
app.get('/deferred-props/back-button/a', (req, res) => {
  ...
  const delay = req.headers['x-luna-partial-data']?.includes('fastProp') ? 100 : 600
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        component: 'DeferredProps/BackButton/PageA',
        props: {
          fastProp: req.headers['x-luna-partial-data']?.includes('fastProp') ? 'Fast prop loaded' : undefined,
          slowProp: req.headers['x-luna-partial-data']?.includes('slowProp') ? 'Slow prop loaded' : undefined,
        },
      }),
    delay,
  )
  ...
  setTimeout(
    () =>
      luna.render(req, res, {
        component: 'DeferredProps/BackButton/PageB',
        props: {
          data: req.headers['x-luna-partial-data']?.includes('data') ? 'Page B data loaded' : undefined,
        },
      }),
    400,
  )
...
app.get('/use-http', (req, res) => luna.render(req, res, { component: 'UseHttp/Index' }))
...
function getDefaultTodos() {
  return [
    { id: 1, name: 'Learn Luna.js', done: true },
...
app.get('/nested-props/rescued-deferred-except', (req, res) => {
  const partialData = req.headers['x-luna-partial-data']?.split(',') ?? []
  const partialExcept = req.headers['x-luna-partial-except']?.split(',') ?? []

  if (!req.headers['x-luna-partial-data'] && !req.headers['x-luna-partial-except']) {
    return luna.render(req, res, {
...
app.get('/nested-props/merge', (req, res) => {
  ...
  luna.render(req, res, {
    ...
    ...(req.headers['x-luna-reset'] ? {} : { mergeProps: ['feed.posts'] }),
  })
})
...
app.get('/nested-props/prepend', (req, res) => {
  ...
  luna.render(req, res, {
    ...
    ...(req.headers['x-luna-reset'] ? {} : { prependProps: ['feed.posts'] }),
  })
})
...
app.get('/nested-props/once', (req, res) => {
  ...
  const shouldResolveLocale = !isLunaRequest || isPartialRequest || !hasPropAlready
...
app.get('/nested-props/deep-merge', (req, res) => {
  ...
  luna.render(req, res, {
    ...
    ...(req.headers['x-luna-reset'] ? {} : { deepMergeProps: ['data'] }),
  })
})
...
app.get('/nested-props/except-dot-props', (req, res) => {
  const exceptData = req.headers['x-luna-partial-except']?.split(',') ?? []
...
app.get('/head/plain-title', (req, res) => luna.renderWithPlainTitle(req, res, { component: 'Head/Dataset' }))
  ...
  const head =
    req.query.override !== undefined
      ? [
          ...
          '<meta data-luna="description" name="description" content="Server default">',
        ]
...
app.get('/server-head/keyed', (req, res) =>
  luna.render(req, res, {
    ...
    props: {
      ...
      head: [
        '<title data-luna="title">Keyed Head A</title>',
        '<meta data-luna="description" name="description" content="Keyed description A">',
        '<link data-luna="canonical" rel="canonical" href="https://example.com/a">',
      ],
    },
  }),
)
...
app.get('/server-head/keyed/next', (req, res) =>
  luna.render(req, res, {
    ...
    props: {
      next: '/server-head/keyed',
      head: [
        '<title data-luna="title">Keyed Head B</title>',
        '<meta data-luna="robots" name="robots" content="noindex">',
        '<meta data-luna="description" name="description" content="Keyed description B">',
        '<link data-luna="canonical" rel="canonical" href="https://example.com/b">',
      ],
    },
  }),
)
...
app.get('/head/title-callback', (req, res) =>
  luna.render(req, res, { component: 'Head/TitleCallback', props: { titleSuffix: 'Account' } }),
)

app.get('/head/reactive', (req, res) =>
  luna.render(req, res, { component: 'Head/Reactive', props: { titleSuffix: 'Dashboard' } }),
)

app.all('*page', (req, res) => luna.render(req, res))
...
showServerStatus(luna.package, adapterPorts[luna.package])

app.listen(adapterPorts[luna.package])

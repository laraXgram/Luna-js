...
test.describe('server head', () => {
  test('it renders server-provided head elements on initial load', async ({ page }) => {
    ...
    await page.waitForSelector('title[data-luna="server-head-0"]', { state: 'attached' })

    const headHTML = await getLunaHeadHTML(page)
    expect(headHTML).toBe(
      '<title data-luna="server-head-0">Server Head Initial</title>' +
        '<meta data-luna="server-head-1" name="description" content="Initial server head description">',
    )
  })

  test('it reconciles server head elements by their explicit data-luna key across navigation', async ({ page }) => {
    await page.goto('/server-head/keyed?withServerHead')
    await page.waitForFunction(
      () => document.querySelector('title[data-luna="title"]')?.textContent === 'Keyed Head A',
    )
    ...
    await expect(description).toHaveAttribute('data-luna', 'description')
    ...
    await page.waitForFunction(
      () => document.querySelector('title[data-luna="title"]')?.textContent === 'Keyed Head B',
    )
  ...
  test('it reads server head elements from a custom prop name', async ({ page }) => {
    ...
    const headHTML = await getLunaHeadHTML(page)
    expect(headHTML).toBe(
      '<title data-luna="server-head-0">Custom Prop Head</title>' +
        '<meta data-luna="server-head-1" name="description" content="Custom prop description">',
    )
  })
  ...
  test('it keeps server head elements after a partial reload that omits the head prop', async ({ page }) => {
    ...
    const response = page.waitForResponse(
      (response) =>
        ...
        response.request().headers()['x-luna-partial-data'] === 'foo' &&
        response.status() === 200,
    )
    ...
    expect(await getLunaHeadHTML(page)).toBe(headHTML)
  })
  ...
  test('it lets page head elements override server head elements with the same key', async ({ page }) => {
    ...
    expect(headHTML).toBe(
      '<title data-luna="server-head-0">Server Head Initial</title>' +
        '<meta name="description" content="Page override" data-luna="description">',
    )
  })
})

...
test.describe('Flash Data', () => {
  ...
  test('fires flash event after a location visit triggers a full page reload', async ({ page }) => {
    ...
    await page.evaluate(() => (window as any).testing.Luna.visit('/flash/location-visit'))
  ...
  test('preserves flash data and fires event when page has InfiniteScroll', async ({ page }) => {
    ...
    await page.evaluate(() => window.testing.Luna.visit('/flash/with-infinite-scroll'))

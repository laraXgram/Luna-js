...
test('will start polling when the component mounts with usePoll', async ({ page }) => {
  ...
  const only = await response.request().headerValue('X-Luna-Partial-Data')
  await expect(only).toBe('custom_prop')

  const only2 = await response2.request().headerValue('X-Luna-Partial-Data')
...
test('it cleans up the poll on unmount and does not leak across rerenders', async ({ page }) => {
  ...
  const pollsAfterMount = await page.evaluate(() => window.testing.Luna.activePolls)
  ...
  const pollsAfterRerenders = await page.evaluate(() => window.testing.Luna.activePolls)
  ...
  const pollsAfterUnmount = await page.evaluate(() => window.testing.Luna.activePolls)
...
test('it preserves validation errors when poll reloads data', async ({ page }) => {
  ...
  await page.waitForResponse(
    (response) => response.request().headers()['x-luna-partial-data'] === 'time' && response.status() === 200,
  )

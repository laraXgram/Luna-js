...
test.describe('Nested', () => {
  ...
  test('can handle nested data', async ({ page }) => {
    ...
    await expect(await page.locator('#repo-name').inputValue()).toEqual('lunajs/luna')
    await expect(await page.locator('#organization-name').inputValue()).toEqual('Luna')
    ...
    await page.fill('#repo-name', 'lunajs/inersha')
    await expect(page.locator('#repo-name')).toHaveValue('lunajs/inersha')
    ...
    await expect(dump.form.organization.repo.name).toEqual('lunajs/inersha')
    await expect(dump.form.organization.name).toEqual('Luna')
...
test.describe('Reserved Keys', () => {
  ...
  test('it logs a console error when using reserved form keys', async ({ page }) => {
    ...
    expect(consoleMessages.messages.some((m) => m.includes('[Luna] useForm()'))).toBe(true)

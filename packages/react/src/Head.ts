...
type LunaHead = FunctionComponent<LunaHeadProps>

const Head: LunaHead = function ({ children, title }) {
  ...
  function ensureNodeHasLunaProp(node: ReactElement<any>) {
    return React.cloneElement(node, {
      'data-luna': node.props['head-key'] !== undefined ? node.props['head-key'] : '',
    })
  }

  function renderNode(node: ReactElement<any>) {
    return renderTag(ensureNodeHasLunaProp(node))
  }
  ...
  function renderNodes(nodes: ReactNode) {
    ...
    if (title && !elements.find((tag) => tag.startsWith('<title'))) {
      elements.push(`<title data-luna="">${escape(title)}</title>`)
    }

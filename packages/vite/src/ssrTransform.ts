 ...
 * SSR Transform
 *
 * Transforms the SSR entry file by wrapping `createLunaApp()` with
 ...
 */
export function findLunaAppExport(code: string): boolean {
  const parsed = ParsedCode.from(code)
  return !!(parsed?.lunaStatement || parsed?.createServerStatement)
}

/**
 * Wrap `createLunaApp()` or `createServer()` with the framework's SSR bootstrap.
 */
export function wrapWithServerBootstrap(
  ...
  if (parsed.lunaStatement) {
    ...
    const { statement, call } = parsed.lunaStatement

 ...
 * that LaraGram can call to render pages server-side.
 *
 * How it works:
 * 1. LaraGram detects that the Vite dev server is running
 * 2. LaraGram POSTs the page data to `/__luna_ssr`
 ...
 * 5. The rendered HTML is returned to LaraGram
 *
...
import { classifySSRError, formatConsoleError } from '@laraxgram/luna/ssrErrors'
import type { ResolvedConfig, ViteDevServer } from 'vite'
import { collectCSSFromModuleGraph } from './css'

/** Options for the SSR dev server and production builds. */
export interface LunaSSROptions {
  /**
...
export const SSR_ENDPOINT = '/__luna_ssr'
...
export function resolveSSREntry(options: LunaSSROptions, config: ResolvedConfig): string | null {
  if (options.entry) {
    if (existsSync(resolve(config.root, options.entry))) {
      return options.entry
    }

    config.logger.warn(`Luna SSR entry not found: ${options.entry}`)

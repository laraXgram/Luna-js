/**
 * AST Utilities for Luna Code Transforms
 *
 * Parses JavaScript/TypeScript code using Vite's built-in `parseAst` (Rollup's parser)
 * and provides methods to find Luna-specific patterns like `createLunaApp()` calls,
...
const INERTIA_APP_FUNCTION = 'createLunaApp'
const CREATE_SERVER_FUNCTION = 'createServer'

/** A top-level `createLunaApp()` expression statement with position info. */
export interface LunaStatement {
  statement: NodeWithPos<ExpressionStatement>
  call: NodeWithPos<SimpleCallExpression>
}

/** Position info for the first argument of a `createLunaApp()` call. */
...
/** Wraps a parsed AST with methods to find Luna-specific patterns. */
export class ParsedCode {
  ...
  /**
   * Find a top-level `createLunaApp()` expression statement, ignoring any
   * `await`, `void`, or `.then()`/`.catch()` wrappers around it.
   *
   * Does not match `export default createLunaApp()`.
   */
  get lunaStatement(): LunaStatement | null {
    for (const node of this.ast.body) {
      if (node.type !== 'ExpressionStatement') {
        continue
      }

      const call = this.unwrapLunaCall(node.expression)
  ...
  /**
   * Unwrap an expression to find a `createLunaApp()` call inside it.
   * Strips away `await`, `void`, and `.then()`/`.catch()` chains.
   */
  private unwrapLunaCall(node: Node): NodeWithPos<SimpleCallExpression> | null {
    // void createLunaApp(...) / await createLunaApp(...)
    if ((node.type === 'UnaryExpression' || node.type === 'AwaitExpression') && node.argument) {
      return this.unwrapLunaCall(node.argument)
    }

    if (node.type === 'CallExpression') {
      if (this.isLunaCall(node)) {
        return node as NodeWithPos<SimpleCallExpression>
      }

      // createLunaApp({}).then(...).catch(...)
      if (node.callee.type === 'MemberExpression') {
        return this.unwrapLunaCall(node.callee.object)
      }
    }
  ...
  /**
   * Find all `createLunaApp()` calls, including those nested inside exports.
   */
  get lunaCalls(): CallExpression[] {
    const calls: CallExpression[] = []

    this.walkAst(this.ast, (node) => {
      if (node.type === 'CallExpression' && this.isLunaCall(node as CallExpression)) {
  ...
  get pagesProperty(): NodeWithPos<Property> | null {
    for (const call of this.lunaCalls) {
  ...
  /** Find a `createLunaApp()` call that has no `pages` or `resolve` property yet. */
  get callWithoutResolver(): { callEnd: number; options?: LunaCallOptions } | null {
    for (const call of this.lunaCalls) {
      const callWithPos = call as NodeWithPos<CallExpression>

      // Empty call: createLunaApp()
  ...
  private isLunaCall(node: CallExpression | SimpleCallExpression): boolean {

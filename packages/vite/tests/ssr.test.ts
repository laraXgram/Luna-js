...
describe('SSR', () => {
  ...
  describe('entry detection', () => {
    it('uses explicit string entry when file exists', () => {
      ...
      const plugin = luna({ ssr: 'custom/ssr.ts' })
      const logger = createMockLogger()

      plugin.configResolved!(createMockConfig(logger, true))

      expect(logger.info).toHaveBeenCalledWith('Luna SSR entry: custom/ssr.ts')
    })

    it('uses explicit object entry when file exists', () => {
      mockExistsSync.mockImplementation((path: string) => path.endsWith('custom/ssr.ts'))

      const plugin = luna({ ssr: { entry: 'custom/ssr.ts' } })
      const logger = createMockLogger()
    ...
    it('warns when explicit entry does not exist', () => {
      ...
      const plugin = luna({ ssr: 'missing/ssr.ts' })
      const logger = createMockLogger()

      plugin.configResolved!(createMockConfig(logger, true))

      expect(logger.warn).toHaveBeenCalledWith('Luna SSR entry not found: missing/ssr.ts')
    })

    it('auto-detects LaraGram SSR entries', () => {
      ...
      for (const entry of entries) {
        ...
        expect(logger.info).toHaveBeenCalledWith(`Luna SSR entry: ${entry}`)
      }
    })

    it('auto-detects non-LaraGram SSR entries', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('Luna SSR entry: src/ssr.ts')
    })
    ...
    it('auto-detects app.ts as fallback SSR entry', () => {
      ...
      expect(logger.info).toHaveBeenCalledWith('Luna SSR entry: resources/js/app.ts')
    })
  ...
  describe('dev server', () => {
    it('adds middleware when entry exists', () => {
      ...
      expect(server.middlewares.use).toHaveBeenCalledWith('/__luna_ssr', expect.any(Function))
      expect(logger.info).toHaveBeenCalledWith('Luna SSR dev endpoint: /__luna_ssr')
    })
  ...
  describe('middleware', () => {
    ...
    it('includes stack trace when formatErrors is enabled', async () => {
      ...
      const plugin = luna({ ssr: { formatErrors: true } })
      const logger = createMockLogger()
      const server = createMockServer(logger)
    ...
    it('throws raw error when formatErrors is disabled', async () => {
      ...
      const plugin = luna({ ssr: { formatErrors: false } })
      const logger = createMockLogger()
      const server = createMockServer(logger)
  ...
  describe('SSR transform', () => {
    it('wraps createLunaApp with server bootstrap for Vue', () => {
      mockExistsSync.mockReturnValue(false)
...
createLunaApp({ resolve: (name) => name })`
    ...
    it('wraps createLunaApp with server bootstrap for Svelte', () => {
      mockExistsSync.mockReturnValue(false)
...
createLunaApp({ resolve: (name) => name })`
    ...
    it('passes SSR config to server', () => {
      ...
      const plugin = luna({ ssr: { port: 13715, cluster: true } })
      const logger = createMockLogger()

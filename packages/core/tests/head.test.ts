...
describe('createHeadManager', () => {
  describe('server head', () => {
    it('normalizes server head elements into Luna managed elements', () => {
      expect(resolveServerHead(page(), () => ['<meta name="description" content="Users">'])).toEqual([
        '<meta data-luna="server-head-0" name="description" content="Users">',
      ])
    })
    ...
    it('can resolve server head elements from a page prop', () => {
      expect(resolveServerHead(page({ head: ['<title>Users</title>'] }), true)).toEqual([
        '<title data-luna="server-head-0">Users</title>',
      ])
    })
    ...
    it('updates server head elements using a reserved provider', () => {
      ...
      const manager = createHeadManager(
        ...
        ['<meta data-luna="description" name="description" content="Users">'],
      )

      manager.updateServerHead(['<meta data-luna="description" name="description" content="Organizations">'])

      expect(collected[collected.length - 1]).toEqual([
        '<meta data-luna="description" name="description" content="Organizations">',
      ])
    ...
    it('allows page head elements to override server head elements with the same key', () => {
      ...
      const manager = createHeadManager(
        ...
        (title) => title,
        (elements) => collected.push(elements),
        ['<meta data-luna="description" name="description" content="Default">'],
      )

      const provider = manager.createProvider()
      provider.update(['<meta data-luna="description" name="description" content="Page-specific">'])

      expect(collected[collected.length - 1]).toEqual([
        '<meta data-luna="description" name="description" content="Page-specific">',
      ])
    })

    it('deduplicates single-quoted data-luna keys', () => {
      const collected: string[][] = []
  ...
  describe('SSR title escaping', () => {
    it('escapes HTML in the title element to prevent XSS injection via newline bypass', () => {
      ...
      const provider = manager.createProvider()
      provider.update([`<title data-luna="">Safe Title\n</title><script>alert('xss')</script></title>`])

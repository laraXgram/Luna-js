...
// the framework-agnostic HTTP client (`http`) to Luna visit internals. Mirrors
 ...
 * `dev` option of `createLunaApp`, so it is inert in production builds.
 *
 * @internal Not part of the public API. May change or be removed without notice.
 */
export function exposeInterceptors(): void {
  ...
  ;(window as Window & { __luna_interceptors__?: VisitInterceptors }).__luna_interceptors__ = interceptors
}

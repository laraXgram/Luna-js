...
import { LunaAppResponse, Page } from './types'
...
type AppCallback = (page: Page) => LunaAppResponse
...
export default (render: AppCallback, options?: Port | ServerOptions): AppCallback => {
  ...
  if (useCluster && cluster.isPrimary) {
    log('Primary Luna SSR server process started...')
  ...
  const handleRender = async (request: IncomingMessage, response: ServerResponse) => {
    ...
    try {
      ...
      response.writeHead(200, { 'Content-Type': 'application/json', Server: 'Luna.js SSR' })
      ...
      response.writeHead(500, { 'Content-Type': 'application/json', Server: 'Luna.js SSR' })
  ...
  createServer(async (request, response) => {
    ...
    response.end()
  }).listen({ port, host }, () => log('Luna SSR server started.'))

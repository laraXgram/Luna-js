...
function ensureElementHasLunaAttribute(element: string, index: number): string {
  if (element.match(/\sdata-luna(=|\s|>)/)) {
    return element
  }

  // Use positional keys by default; callers can provide `data-luna` for stable identity.
  return element.replace(/^<([a-zA-Z][^\s/>]*)/, `<$1 data-luna="server-head-${index}"`)
}
...
export function resolveServerHead(page: Page, serverHead?: ServerHeadOption): string[] {
  ...
  return elements
    ...
    .map(ensureElementHasLunaAttribute)
}
...
const Renderer = {
  ...
  isLunaManagedElement(element: Element): boolean {
    return element.nodeType === Node.ELEMENT_NODE && element.getAttribute('data-luna') !== null
  },

  findMatchingElementIndex(element: Element, elements: Array<Element>): number {
    const key = element.getAttribute('data-luna')
    if (key !== null) {
      return elements.findIndex((element) => element.getAttribute('data-luna') === key)
    }
  ...
  update: debounce(function (elements: Array<string>) {
    ...
    const targetElements = Array.from(document.head.childNodes).filter((element) =>
      this.isLunaManagedElement(element as Element),
    )

    if (sourceElements.some((element) => element instanceof HTMLTitleElement)) {
      document.head.querySelectorAll('title:not([data-luna])').forEach((title) => title.remove())
    }
...
export default function createHeadManager(
  ...
  function collect(): Array<string> {
    ...
    const defaults: Record<string, string> = {
      ...(title ? { title: `<title data-luna="">${title}</title>` } : {}),
    }
    ...
    const elements = serverHead.concat(providerHead).reduce((carry, element) => {
      ...
      const match = element.match(/ data-luna=(["'])[^"']+\1/)
      if (match) {

...
function addEventListeners(delay: number): void {
  document.addEventListener('luna:start', (e) => handleStartEvent(e, delay))
  document.addEventListener('luna:progress', handleProgressEvent)
}
...
function handleStartEvent(event: GlobalEvent<'start'>, delay: number): void {
  ...
  document.addEventListener('luna:finish', (e) => finish(e, timeout), { once: true })
}

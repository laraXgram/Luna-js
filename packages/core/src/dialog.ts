...
export default {
  createIframeAndPage(html: Record<string, unknown> | string): { iframe: HTMLIFrameElement; page: HTMLElement } {
    if (typeof html === 'object') {
      html = `All Luna requests must receive a valid Luna response, however a plain JSON response was received.<hr>${JSON.stringify(
        html,
      )}`
    }
  ...
  show(html: Record<string, unknown> | string): void {
    ...
    dialogStyleElement.textContent = `
      dialog#luna-error-dialog::backdrop {
        background-color: rgba(0, 0, 0, 0.6);
      }

      dialog#luna-error-dialog:focus {
        outline: none;
      }
    `

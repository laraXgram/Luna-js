...
export class RequestParams {
  ...
  public headers(): HttpRequestHeaders {
    ...
    if (this.isPartial()) {
      headers['X-Luna-Partial-Component'] = currentPage.get().component
    }

    const only = this.params.only.concat(this.params.reset)

    if (only.length > 0) {
      headers['X-Luna-Partial-Data'] = only.join(',')
    }

    if (this.params.except.length > 0) {
      headers['X-Luna-Partial-Except'] = this.params.except.join(',')
    }

    if (this.params.reset.length > 0) {
      headers['X-Luna-Reset'] = this.params.reset.join(',')
    }

    if (this.params.errorBag && this.params.errorBag.length > 0) {
      headers['X-Luna-Error-Bag'] = this.params.errorBag
    }
